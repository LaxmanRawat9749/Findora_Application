...
def build_final_findora_documentation():
    src_file = "Final_Document_Format_Complete_Diagrams.backup.docx"
    dst_file = "findora_documentation.docx"

    shutil.copyfile(src_file, dst_file)
    print(f"Loaded base template from {src_file}")
    ...
    # Remove any rows starting with Figure 6.2.
    rows_to_delete = []
    ...
    print("Table 4 (List of Figures) updated with all 13 figures.")

    # 2. Locate Chapter 6 and Chapter 7 insertion targets
    p_ch6_2_idx = None
    p_ch7_idx = None
    ...
    print(f"Indices: Ch6.2={p_ch6_2_idx}, Ch7={p_ch7_idx}")
    target_p = doc.paragraphs[p_ch7_idx]

    # Insert Section 6.2
    target_p.insert_paragraph_before("6.2 Screen-by-Screen User & Administrator Interface Documentation", "Heading 2")
    ...
    target_p.insert_paragraph_before("6.3 Operational Workflow and Comprehensive User Guidelines", "Heading 2")
    
    p_wf1 = target_p.insert_paragraph_before(
    ...
    workflows = [
        ("1. User Registration & Security Onboarding", "New users create a verified account with legal name, contact credentials, and avatar. Session tokens are securely cached for seamless app launches using EncryptedSharedPreferences and JWT tokens."),
    ...
    for p in doc.paragraphs:
        if p.text.strip() == "6.2 App Snapshots":
            record = True
        if p.text.strip() == "6.2 Screen-by-Screen User & Administrator Interface Documentation":
            record = False
        if record:
            to_remove.append(p)
    ...
    doc.save(dst_file)
    print(f"Generated final {dst_file} successfully!")

if __name__ == "__main__":
    build_final_findora_documentation()
