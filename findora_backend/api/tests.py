...
from api.views import (
    ...
    AdminItemListView, AdminVerifyItemView, ProfileView, PublicProfileView
)
...
class ItemMatchingAndRoleVisibilityTests(TestCase):
    ...
    def test_unapproved_found_item_hidden_from_other_users(self):
        ...
        force_authenticate(req, user=self.owner)
        res = view(req, pk=pending_item.id)
...
class LostItemAdminApprovalWorkflowTests(TestCase):
    ...
    def test_finder_submits_found_item_saved_as_pending_and_approval_workflow(self):
        ...
        # 6. After approval: Found item is visible on Finder dashboard and Owner dashboard
        res_f_after = items_view(req_f)
        self.assertIn(found_id, [it['id'] for it in res_f_after.data])

        # Approved found items are visible on Owner dashboard
        res_o_after = items_view(req_o)
        self.assertIn(found_id, [it['id'] for it in res_o_after.data])

        # 7. When a conversation is established between Finder and Owner, Found item remains visible
...
class CrossUserFoundItemIndependenceTests(TestCase):
    """
    Verifies that an Owner sees their own Lost items and all approved Found items,
    while another Owner's Lost items are NOT shown.
    """
    ...
    def test_owner2_lost_laptop_does_not_see_owner1_lost_laptop_but_sees_finder1_found_laptop(self):
        """
        ...
        5. Owner2's dashboard shows Owner2's Lost Laptop and Finder1's Found Laptop, but NOT Owner1's Lost Laptop.
        """
        items_view = ItemListCreateView.as_view()
...
class OwnerDashboardFinderFoundReportsVisibilityTests(TestCase):
    ...
    def test_unapproved_found_reports_remain_hidden_from_owner(self):
        ...
        # Detail view for unapproved items returns 403
        req_pending = self.factory.get(f'/api/items/{pending_found.id}/')
        force_authenticate(req_pending, user=self.owner1)
        res_pending = self.item_detail_view(req_pending, pk=pending_found.id)
...
class FinderRecoveredItemsCountAndProfileTests(TestCase):
    """
    Test suite verifying Finder Profile 'No. of Recovered Items' count and duplicate protection.
    """

    def setUp(self):
        self.factory = APIRequestFactory()
        self.finder = User.objects.create_user(
            username='finder_john',
            email='finder_john@test.com',
            password='Password123!',
            role='finder',
            is_verified=True,
        )
        self.owner = User.objects.create_user(
            username='owner_jane',
            email='owner_jane@test.com',
            password='Password123!',
            role='owner',
            is_verified=True,
        )
        self.profile_view = ProfileView.as_view()
        self.rep_view = ReputationProfileView.as_view()
        self.public_profile_view = PublicProfileView.as_view()
        self.mark_return_view = MarkItemReturnedView.as_view()
        self.confirm_return_view = ConfirmItemReturnView.as_view()
        self.my_reports_view = MyReportsView.as_view()

    def test_zero_completed_returns_returns_zero(self):
        """1. Finder with 0 completed returns -> No. of Recovered Items = 0."""
        req_profile = self.factory.get('/api/profile/')
        force_authenticate(req_profile, user=self.finder)
        res_profile = self.profile_view(req_profile)
        ...
        req_rep = self.factory.get('/api/reputation/me/')
        force_authenticate(req_rep, user=self.finder)
        res_rep = self.rep_view(req_rep)
        self.assertEqual(res_rep.status_code, 200)
        self.assertEqual(res_rep.data['items_recovered'], 0)
        self.assertEqual(res_rep.data['successful_returns'], 0)

        req_pub = self.factory.get(f'/api/users/{self.finder.id}/public-profile/')
        force_authenticate(req_pub, user=self.owner)
        res_pub = self.public_profile_view(req_pub, pk=self.finder.id)
        self.assertEqual(res_pub.status_code, 200)
        self.assertEqual(res_pub.data['items_recovered'], 0)
        self.assertEqual(res_pub.data['recovered_items'], 0)

    def test_completing_one_valid_return_increments_to_one(self):
        """2. Finder completes 1 valid return -> count = 1."""
        ...
        item1.resolved_at = timezone.now()
        item1.save()
        ...
        force_authenticate(req_rep, user=self.finder)
        res_rep = self.rep_view(req_rep)
        self.assertEqual(res_rep.data['items_recovered'], 1)
        self.assertEqual(res_rep.data['successful_returns'], 1)

    def test_completing_multiple_different_returns_increments_correctly(self):
        """3 & 4. Finder completes another return -> count = 2. Reopening still shows 2."""
        item1 = Item.objects.create(
            user=self.finder,
            type='found',
            title='Found Keys',
        ...
        PointTransaction.objects.create(
            user=self.finder,
            points=100,
            transaction_type='SUCCESSFUL_RETURN',
            description='Successful return of Lost iPhone',
            related_item=lost_item,
        )

        for _ in range(2):  # Simulating reopening/refreshing profile
            req_profile = self.factory.get('/api/profile/')
            force_authenticate(req_profile, user=self.finder)
            res_profile = self.profile_view(req_profile)
            self.assertEqual(res_profile.data['items_recovered'], 2)
            self.assertEqual(res_profile.data['successful_returns'], 2)

            req_rep = self.factory.get('/api/reputation/me/')
            force_authenticate(req_rep, user=self.finder)
            res_rep = self.rep_view(req_rep)
            self.assertEqual(res_rep.data['items_recovered'], 2)
            self.assertEqual(res_rep.data['successful_returns'], 2)

    def test_duplicate_confirmations_and_notifications_do_not_increase_count(self):
        """5. Duplicate confirmations/notifications for the same return do NOT increase count."""
        item = Item.objects.create(
            user=self.finder,
            type='found',
            title='Found Wallet',
        ...
        # Multiple notifications for same return
        Notification.objects.create(user=self.finder, type='claim', message='Return request 1', related_item=item)
        Notification.objects.create(user=self.finder, type='claim', message='Return request 2', related_item=item)
        Notification.objects.create(user=self.finder, type='reputation', message='Reputation update', related_item=item)

        # Multiple point transactions for same item (if any audit/adjustment existed)
        PointTransaction.objects.create(
            user=self.finder,
            points=100,
            transaction_type='SUCCESSFUL_RETURN',
            description='Successful return 1',
            related_item=item,
        )
    ...
    def test_pending_active_cancelled_rejected_returns_not_counted(self):
        """6. Pending, active, and rejected returns must NOT increase count."""
        Item.objects.create(user=self.finder, type='found', title='Pending Item', category='other', status='pending')
        Item.objects.create(user=self.finder, type='found', title='Active Item', category='other', status='approved')
        Item.objects.create(user=self.finder, type='found', title='Rejected Item', category='other', status='rejected')

        # Active return with only owner confirmed
        Item.objects.create(
            user=self.finder,
            type='found',
            title='Partially Confirmed Item',
            category='other',
            status='approved',
            owner_returned_confirm=True,
            finder_returned_confirm=False,
        )
    ...
    def test_owner_profile_unaffected(self):
        """Owner Profile behavior remains unaffected (shows lost reports, 0 recovered items)."""
        Item.objects.create(user=self.owner, type='lost', title='Owner Lost Item', category='wallet', status='approved')

        req_profile = self.factory.get('/api/profile/')
        force_authenticate(req_profile, user=self.owner)
        res_profile = self.profile_view(req_profile)
        self.assertEqual(res_profile.data['lost_reports'], 1)
        self.assertEqual(res_profile.data['items_recovered'], 0)
        self.assertEqual(res_profile.data['successful_returns'], 0)

    def test_my_reports_filter_recovered_items(self):
        """MyReportsView returns all and only successfully recovered items for Finder."""
        resolved_item = Item.objects.create(
            user=self.finder, type='found', title='Resolved Item', category='keys', status='resolved',
            owner_returned_confirm=True, finder_returned_confirm=True, resolved_at=timezone.now()
        )
        active_item = Item.objects.create(
            user=self.finder, type='found', title='Active Item', category='keys', status='approved'
        )

        req = self.factory.get('/api/profile/items/?filter=items_recovered')
        force_authenticate(req, user=self.finder)
        res = self.my_reports_view(req)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(len(res.data), 1)
        self.assertEqual(res.data[0]['id'], resolved_item.id)
