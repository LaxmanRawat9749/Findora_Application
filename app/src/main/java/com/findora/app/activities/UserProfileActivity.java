...
public class UserProfileActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Cache-First: Display cached public profile immediately
        PublicProfile cached = FindoraCache.getInstance(this).getCachedPublicProfile(userId);
        if (cached != null) {
            populateUI(cached);
        }

        loadProfile(cached != null);
    }

    private void loadProfile(boolean hasCachedData) {
        if (!hasCachedData) {
        ...
        apiService.getPublicProfile(userId).enqueue(new Callback<PublicProfile>() {
            ...
            public void onResponse(Call<PublicProfile> call, Response<PublicProfile> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    PublicProfile profile = response.body();
                    FindoraCache.getInstance(UserProfileActivity.this).savePublicProfile(userId, profile);
                    populateUI(profile);
    ...
    private void populateUI(PublicProfile profile) {
        ...
        if (profile.getProfileImage() != null && !profile.getProfileImage().isEmpty()) {
            ...
            GlideImageHelper.loadAvatar(this, profile.getProfileImage(), binding.ivProfilePicture);
        } else {
